...
rootdir=$(pwd)
# 0 获取安装flag
if [ -n "$( echo $@ | grep "sba")" ];then
        sbaf=1
        sbap="sba"
fi
if [ -n "$( echo $@ | grep "elk")" ];then
        elkf=1
        elkp="elk"
fi
if [ -n "$( echo $@ | grep "zabbix")" ];then
        zabbixf=1
        zabbixp="zabbix"
fi
echo "计划安装:" ${sbap} ${elkp} ${zabbixp}
echo "----------安装开始----------"
...
nat2bridge(){
        cd ${rootdir}/docker
        rpm -ivh net-tools-2.0-0.6.20130109git.fc19.x86_64.rpm --nodeps --force && rpm -ivh bridge-utils-1.5-9.el7.x86_64.rpm --nodeps --force
        if [ $? == 0 ];then
                pkill docker
                iptables -t nat -F
